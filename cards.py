...
class Deck( object ):
    ...
    def euchre_deck( self, size=24 ):
        """ return a euchre deck with size size. 
            size could only be:
                32 - A, K, Q, J, 10, 9, 8, 7 of each suit
                28 - 7s of each suit omitted
                24 - 7s and 8s omitted (default)
        """
        if size not in [24, 28, 32]:
            return None
        else:
            num = 14 - (size - 4) // 4 #find the lowest rank for the deck
            self.__deck = []
            for s in range(1,5):
                self.__deck.append(Card(1,s))
                for r in range(num,14):
